...
#                             PR provenance examples may use the local driver
#                             name (`Claude` vs `Codex`) and are normalized.
...
normalize_driver_specific_lines() {
  # Skill bodies should stay aligned, but provenance examples are read by the
  # local driver and intentionally name that driver. Normalize only that line.
  awk '
    /Do not identify the driving tool as PR provenance/ {
      gsub(/\[claude\]/, "[driver]")
      gsub(/\[codex\]/, "[driver]")
      gsub(/chore\(claude\)/, "chore(driver)")
      gsub(/chore\(codex\)/, "chore(driver)")
      gsub(/Generated by Claude/, "Generated by Driver")
      gsub(/Generated by Codex/, "Generated by Driver")
      gsub(/Created by Claude/, "Created by Driver")
      gsub(/Created by Codex/, "Created by Driver")
      gsub(/Co-authored-by: Claude/, "Co-authored-by: Driver")
      gsub(/Co-authored-by: Codex/, "Co-authored-by: Driver")
      gsub(/Mention Claude only/, "Mention Driver only")
      gsub(/Mention Codex only/, "Mention Driver only")
      gsub(/Claude behavior or configuration/, "Driver behavior or configuration")
      gsub(/Codex behavior or configuration/, "Driver behavior or configuration")
    }
    { print }
  '
}
...
while IFS= read -r skill; do
  ...
  normalize_body "$cl_body.raw" | normalize_driver_specific_lines > "$cl_body"
  normalize_body "$cx_body.raw" | normalize_driver_specific_lines > "$cx_body"
