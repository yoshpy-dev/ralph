...
4. Create a ready-for-review PR with `gh pr create` using [template.md](template.md) for the body structure. **PR title and body must be written in Japanese.** Do not pass `--draft` unless the operator explicitly requests a draft.
...
5. After PR creation, run `./scripts/ensure-pr-ready.sh <pr-url-or-current-branch>`. If it reports that the PR is still draft, stop and do NOT report the PR as complete.
