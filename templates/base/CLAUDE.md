...
- `/plan` asks at minimum one decision — standard flow (/work) or Ralph Loop (/loop) — and, when critical forks are detected during drafting (two+ approaches with materially different risk/cost that cannot be resolved from repo context), asks targeted AskUserQuestion follow-ups before finalizing.
- `/work` creates a normal branch (`git checkout -b`) and starts interactive implementation. Post-impl pipeline runs via subagents.
- `/loop` uses a directory-based plan and runs `ralph-orchestrator.sh` for autonomous parallel-slice execution: multi-worktree → `ralph-pipeline.sh` per slice → integration branch → sequential merge → integration pipeline (`--skip-pr --fix-all`) → unified PR.
