...
| **Leaded** | 1 | Lead が reviewer もしくは qa を 1 座席立て、レビュー/検証だけを座席に委譲する。実装は完了済みか既存フロー(`/work`)で進める前提で、Lead 自身は実装しない(`lead.md` の implementer 優先委譲と同じ立場)。 | 実装は完了しているが第三者視点のレビューやテスト実行が要る場合。 |
