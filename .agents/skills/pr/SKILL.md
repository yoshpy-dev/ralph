...
3. **Scan the branch history for secrets, right before pushing**: run `./scripts/secret-scan-branch.sh --strict`. It scans the same range CI scans (merge-base with the base branch to `HEAD`) using the `.gitallowed` committed at `HEAD`, so run it after the last commit. Exit 0: continue. Exit 1 (findings): do NOT push. History is scanned, so deleting the line in a new commit does not clear it: either rewrite the unpushed commits, or add a narrow `.gitallowed` regex (written so that the line itself does not match a scanner pattern), commit it, and re-run. Exit 3 (could not scan: no base ref, no merge-base, scanner missing): do NOT push; fix the cause (for example `git fetch origin <base>`) and re-run. Run it again before every later push to this branch (for example the push that carries the plan-archive commit).
...
8. Archive the plan using the path resolved in Step 1: `./scripts/archive-plan.sh <absolute-plan-path>`.
...
- [ ] `./scripts/secret-scan-branch.sh --strict` exited 0 on the pushed HEAD
