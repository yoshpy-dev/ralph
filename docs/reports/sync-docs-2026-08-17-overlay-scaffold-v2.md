...
- Scope: delta since cycle-1 (`4c38cf1..b39cd14`) — cross-review triage (`ba2384f`), cross-review fixes 1ef5be7 (new `ReplacePlan.ManifestRemove` field in `internal/upgrade/replaceplan.go`, `ApplyOps` upfront path validation, `WriteUpgradeReport` report-path-prefix guard), cycle-2 self-review append (`c81eee2`), cycle-2 self-review cleanup (`3b32c50`: `AdvisoryEntry`/`ApplyOps` doc-comment fixes, `versionSanitizeRe` → `reportNameSanitizeRe` rename + `WriteUpgradeReport` guard consolidation, two `docs/tech-debt/README.md` rows, root-skip guard for the `chmod 0444` LOW-7 test, insight-event backfill), cycle-2 verify append (`aecd254`), cycle-2 test append (`b39cd14`).
- Checked, no drift found:
  - **AGENTS.md repo map** — `internal/upgrade/` line already names the primitives generically ("core replace planner with ordered ops + commit barrier"), not individual struct fields; the new `ManifestRemove` field is exactly the kind of implementation detail the cycle-1 doc-maintainer verdict already excluded from this map. No update needed.
  - **`docs/tech-debt/README.md`** — the two rows `3b32c50` added (`KNOWN_DIFFS` whole-file-suppression scope note; 5 unfixed cycle-1 LOW findings) are internally consistent: both cite `docs/reports/self-review-2026-08-17-overlay-scaffold-v2.md` Cycle 2 sections and existing file paths that all resolve. No table-format or cross-reference drift.
  - **Plan/spec cross-references** — `docs/plans/active/2026-08-17-overlay-scaffold-v2.md`'s Deviations section now documents the `versionSanitizeRe`→`reportNameSanitizeRe` rename and the `KNOWN_DIFFS` addition inline; both match the actual commits. `docs/specs/2026-08-17-overlay-scaffold-v2.md` stays at requirements/acceptance-criteria abstraction (no function- or field-level detail — confirmed via section-header scan and targeted greps for "delete"/"classifyCore"/manifest-removal language, all zero hits), so `ManifestRemove` and the `ApplyOps`/`WriteUpgradeReport` hardening are below the spec's documented level of detail and don't require a spec edit.
  - **Stale symbol names** — repo-wide grep for `versionSanitizeRe` and `managedForOwner` returns zero hits in `.go` files; all remaining `.md` hits are inside `docs/reports/self-review-*.md` and `docs/reports/verify-*.md` (correctly-preserved point-in-time records of the rename) and one `docs/plans/active/2026-08-17-overlay-scaffold-v2.md` Deviations line that itself narrates the rename (accurate, not stale).
- Verification re-run after the delta: `./scripts/check-sync.sh` → PASS (`DRIFTED: 0`, `KNOWN_DIFF: 4`, unchanged from cycle 1); `./scripts/check-skill-sync.sh` → PASS (13 skills in lock-step); `./scripts/check-pipeline-sync.sh` → PASS (all 5 pipeline-order reference files in sync).
- Verdict: **no drift**. No files required changes beyond this report.
- Files changed this cycle: `docs/reports/sync-docs-2026-08-17-overlay-scaffold-v2.md` (this report) only.
