...
- 最終の cross-review(cycle 2、cap 到達)で出た 2 件は未修正で、後続 #173 で直す(ユーザー決定)。(1) stop 時の観測は、比較する指定モデルを roster から取る。spawn の中断 → 別モデルでの再試行が拒否 → stop の順で、実際には指定どおり動いている座席に誤った `honored=false` と警告が出る。(2) 観測の 1 回の走査は期限も ctx の取り消しも見ない。spawn 以降に更新された大きな記録が多数あると、既定の 8 秒と `--timeout-ms` を超える(Codex の再現: 約 3.8 MiB × 50 件で 9.45 秒。通常は数件・数十 ms)。
