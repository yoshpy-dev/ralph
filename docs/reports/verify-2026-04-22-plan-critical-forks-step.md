...
- Partially verified:
  - Cross-reference `[template.md](../../.claude/skills/plan/template.md)` on SKILL.md:51 resolves to the sibling fallback only; this is safe because every real template carries the section, but the pointer itself is not a precise landing reference. Matches self-review L1 (LOW).
