...
- Evidence: `docs/evidence/verify-2026-05-18-021712.log`

## Acceptance Criteria

| Criterion | Verdict | Evidence |
| --- | --- | --- |
| Existing v1 manifests read successfully and do not force hunk UI when baseline is missing | PASS | `TestReadManifestV1Compatibility`, `TestRunUpgrade_V1ManifestConflict_UsesLegacyPrompt` |
...
| Baseline-available conflicts expose only `apply / keep / edit / skip file`; no `next` or `quit` | PASS | `TestRunUpgrade_InteractiveDiff_RepromptsOnInvalid` asserts prompt shape and absence of `next`/`quit` |
| Full hunk edit/final-summary semantics are tracked as follow-up debt | PASS | `docs/tech-debt/README.md` entry added |
| Tests cover manifest compatibility, baseline writing, dry-run behavior, and hunk prompt option shape | PASS | New/updated tests in `internal/scaffold` and `internal/cli` |
...
Updated `docs/specs/2026-04-16-ralph-cli-tool.md` with manifest v2 baseline metadata, `--dry-run`, `--diff`, `--pager`, and hunk-prompt fallback semantics. Added tech debt for the remaining #97 editor/merge work.
