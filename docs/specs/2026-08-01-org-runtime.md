...
- **(a) 同時に変える面**: 既定は 3 面(`internal/config/config.go` の `Default()`、`templates/base/ralph.toml` の `model_pool`、`scripts/ralph-config.sh` の `RALPH_ORG_MODEL_POOL`)のロックステップで、`internal/config/defaults_sync_test.go` がこの 3 面の一致を検査する。`templates/base/scripts/ralph-config.sh` は root との byte 一致を `scripts/check-sync.sh` が検査する(テストは読まない)。加えて `.claude/skills/org/SKILL.md` の「既定の model_pool」表(`scripts/sync-skills.sh` で `.agents/skills/` を再生成し、`templates/base/.claude/skills/` と `templates/base/.agents/skills/` へ cp した 4 面)、本 spec の 2026-09-16 改訂注記 (c)、既定値をハードコードするテスト(`internal/config/config_test.go`、`internal/org/envelope_summary_test.go`、`internal/cli/org_test.go`)を追従させる。この列挙は閉じていないので、最後に `git grep -n '<旧スラッグ>'` で残存を掃く(`docs/insights/events/`・`docs/reports/`・`docs/plans/archive/` は履歴なので除外)。2026-09-18 時点で列挙の外にある例: `.claude/skills/org/SKILL.md`「前提」節の「プール先頭(claude は `fable`、codex は `gpt-6-astra`)」の記述(4 面)、`templates/base/ralph.toml` の `[org.roles]` コメント例。どのゲートも捕まえない(lock-step test は値、sync 系は写し同士、doctor は cache との比較)。
- **(b) 検証**: `./scripts/run-verify.sh`(`defaults_sync_test` と `check-sync.sh` / `check-skill-sync.sh` を含む)と `ralph doctor` が pass すること。
- **(c) 配布は 2 段階**: `/release` でタグを切ったあと、下流は **まずバイナリを更新**(`brew update && brew upgrade ralph`、または各自のインストール経路)し `ralph version` で新バージョンを確認してから、**その後** `ralph upgrade` で core(skill・`ralph-config.sh`)を置換する。`ralph upgrade` は `cmd/ralph/main.go` が `scaffold.EmbeddedFS` に注入した「今入っているバイナリの」埋め込みテンプレートを適用するため、バイナリ更新を飛ばすと旧既定・旧 core のまま成功したように見える。seed-once の `ralph.toml` はどちらの段階でも更新されないが、`ralph upgrade` は埋め込みテンプレートのハッシュが記録と異なる seed ファイルについて advisory diff を `docs/reports/upgrade-*.md` に出す(`internal/upgrade/replaceplan.go` の `AdvisoryEntry`)ので、下流では新しい既定 `model_pool` がその diff として見える。下流運用者はそれを入口に `/org` skill の「既定の model_pool」節の手順で自分の `model_pool` を直す。
- **(d) 観測手順と消失の判定基準**: `checkCodexModelSlugs` はローカルの `models_cache.json` を読むだけで更新はしない(書き込み時刻 `cache written <UTC>` と、24 時間より古い場合の stale 注記は Detail に出る — issue #159)。観測は毎回 **cache を更新してから** 行う — `codex exec --sandbox read-only 'echo ok' </dev/null` 等で codex を一度起動(codex 起動で cache が書き直され mtime が進む。2026-09-18 に確認)→ `ralph doctor` の Detail に出る `cache written`(UTC 表記。JST なら +9 時間)が観測時刻に更新されていることを確認する(`stat` でも可)。更新直後なら同じ Detail の `<age> ago` が `0m` になるので、時差を考えずに済む → issue #156 のコメントに「日時 / cache mtime / `codex --version` / モデル数 / 既定 5 スラッグの有無 / doctor 結果」を記録する。mtime が更新されていない観測は不成立(inconclusive)として数えない。一時的な消失と区別するため、更新済み cache で 2 週間連続して消えたままなら既定から外す(または後継スラッグへ置換する)判断に入り、迷う場合は 4 週間まで観測を延長する。それ未満では外さない。
...
- [ ] **予算執行の決定論性** **(2026-09-16 改訂で撤去)**: budget の床は wall-clock・座席数・ラウンド数で機構が厳格執行。トークン量は receipts / セッションログから best-effort 計測し insights に記録(遮断条件にはしない)。
