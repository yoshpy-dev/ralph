...
標準フロー(/work)と Ralph Loop(/loop)を廃止し、Lead LLM が herdr(実行・観測)と agmsg(メッセージング)を使って常駐 LLM 座席(scout / impl / reviewer / QA / watchdog)による組織を自律編成する単一の実行面「org runtime」に置き換える。人間の役割はエンベロープ定義(ralph.toml)・Lead 起動・エスカレーション裁定の3つに縮小し、決定論的スクリプトは機構(`ralph org` 動詞)と保証(品質ゲート・ハードリミット・監査証跡)に退く。
...
- [ ] **FR-6 /plan の Lead 吸収と /spec の存続**: `/plan` スキルは廃止し、タスク分解・計画立案は Lead の業務とする。計画アーティファクト(目標・分解・受け入れ条件・座席割当)は監査用に Lead が自動生成し `docs/plans/` に記録する(人間承認なし)。`/spec` は曖昧な要求の言語化フェーズとして存続する。
...
- [ ] **FR-8 Watchdog 二層**: パルス層(決定論タイマー、既定 60 秒: heartbeat 途絶 / プロセス生存 / budget / スコープ外変更 / ラウンド cap。ハードリミット超過は判断を経由せず自動遮断)+ ウォッチャー層(LLM 常駐座席: 循環議論・役割逸脱・偽進捗の意味判定。パルス層の監視対象に含める)。通知は Lead 宛。デッドマン条項: 異常主体が Lead 自身、または Lead が通知に N 分無応答の場合のみ人間へエスカレーション(端末 + PushNotification)。権限は PAUSE / REPLAN 要求 / 通知のみ。
- [ ] **FR-9 監査証跡**: org manifest(`.harness/state/org/manifest.jsonl`)へ全 spawn / send / stop / disband / 遮断イベントを動詞が自動追記(Lead の自己申告に依存しない)。完了時に編成履歴を `docs/reports/` へ成果物化。model receipts(requested / effective / honored)を org 動詞に引き継ぐ。insights スキーマを拡張(リードタイム、初回 CI 成功率、レビュー往復数、人間介入数、座席数、停滞率)。
...
- [ ] **FR-11 旧系の完全撤去**(段階移行なし・PR 系列の最終 PR で一括削除): `ralph-orchestrator.sh`、`ralph-pipeline.sh`、loop-init、旧 shell CLI の loop 系コマンド、`/work` `/loop` `/cross-review` `/self-review` の各スキル、`.claude/agents/` の pipeline サブエージェント定義、loop 系テンプレート・レシピ・rules、`internal/ui` の Bubble Tea TUI(ライブビューは herdr に委譲)、`internal/state` のスライス/checkpoint リーダー、`internal/action` retry/abort。`ralph status` は org manifest を読むテキスト座席ビューに書き換える。`run-static-verify.sh` / `run-test.sh` / `ralph-worktree.sh` / driver 起動ロジックは QA 座席・動詞側に転用して存続。
...
- **越権防止**: プール外モデル・座席上限・スコープ外書き込みは LLM から見えない機構層(動詞バリデーション + hook)で拒否する。エンベロープの壁は判断対象にしない。
