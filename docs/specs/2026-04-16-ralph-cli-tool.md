...
- **ローカル編集検知と `Managed=false` 収束 (local-edit detection & user-owned convergence)**: テンプレートが未変更 (`newHash == manifestHash`) でもディスク内容が記録済み disk hash と乖離している場合は `ActionConflict` として扱う。baseline が missing/空、または `baseline_path` が読めない v1 互換ケースでは従来どおり `[o]verwrite / [s]kip / [d]iff` を提示する。`[d]iff` 選択時は unified diff（`--- local` / `+++ template (version)`、`-` はローカル行・`+` はテンプレート行）を表示する。各変更行には `<旧行番号> <新行番号> │ <prefix><内容>` 形式の右寄せ行番号ガッター（最小2桁、ファイル長に応じて動的拡張）が付与される。CLI の通常 UI では hunk header と template/local hash summary は表示しない。出力先が TTY かつ `NO_COLOR` 環境変数が未設定（空文字）の場合のみ ANSI カラー（`---` 太字赤 / `+++` 太字緑 / `-` 行赤 / `+` 行緑）が付与される。`NO_COLOR=1` を含む `NO_COLOR` の任意の非空値、またはパイプ／ファイルへのリダイレクト時はカラーが抑制される（https://no-color.org 準拠）。legacy `skip` または hunk UI の `skip file` 選択時はマニフェストへ `{Hash: diskHash, Managed: false}` を書き込み、以降そのエントリは `ComputeDiffsWithManifest` で早期 `ActionSkip`（プロンプトも auto-update も抑制）となるため、同一バージョンの再実行で prompt storm が起きない。`overwrite` / template 全適用はローカルをテンプレートへ揃え、マニフェストを `{Hash: newHash, Managed: true}` に戻す。`--force` は `ActionConflict` 経路の上書きに加え、既に `Managed=false` となっているエントリに対してもテンプレート内容をディスクへ書き込み、マニフェストを `{Hash: newHash, Managed: true}` に flip させる（tree 全体を対象とした再管理化エスケープハッチ）。ただしテンプレート側から該当ファイルが削除されている場合は `--force` でも書き戻す元データが無いため `ActionSkip` が維持され、`Managed=false` エントリはディスクとマニフェスト双方で保全される（user-owned 契約がテンプレート変更を跨いで生存）。ディスク読み取り失敗時は警告のみを出し、abort しない。特定パスのみを対象とする `--resync <path>` / `--adopt` は将来スコープ (`docs/tech-debt/README.md` を参照)。
- **manifest v2 baseline metadata と hunk-level partial apply**: 新規 init または upgrade でテンプレート内容を受け入れた managed ファイルは、v1 互換の `hash` / `managed` に加えて `state = "managed"`、`template_hash`、`disk_hash`、`baseline_status = "available"`、`baseline_path = ".ralph/baseline/<path>"` を記録する。baseline が available な conflict では old template / local / new template の line-based 3-way merge planner で hunk を作り、`[a]pply template hunk / [k]eep local hunk / [e]dit / [s]kip file` を提示する（`next` / `quit` は提示しない）。`keep local hunk` は現在の hunk だけをローカル側にし、ファイルは `state = "partial"`、`disk_hash = <resolved content hash>` の managed entry として残る。`edit` は `$VISUAL`、なければ `$EDITOR` で現在 hunk の一時ファイルを開き、editor 未設定・失敗時は target file を変更せず再プロンプトする。hunk 選択はメモリ上に staging され、最終的な `Apply summary` と `Apply these changes? [y/N]` に yes した場合だけ target file、baseline cache、manifest を更新する。`N` / EOF では target file、baseline cache、manifest を変更しない。`ralph upgrade --dry-run` は computed action summary を表示して target files、baseline cache、manifest、git hooks を変更しない。`ralph upgrade --diff` は `--dry-run` を含意し、conflict/auto-update の差分を表示する。`--pager auto|always|never` は diff 表示の pager 使用を制御し、pager 起動失敗時は stdout へフォールバックする。
