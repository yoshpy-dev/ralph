...
- [ ] AC-3: `driver_pool` に codex なし → `pass`、理由なし(`codex_verified` が false / edits・autonomous に解決される role がない / guarded に解決される role がない / `sandbox_mode` が workspace-write でない)→ `pass`、覆っている → `pass`、覆っていない → `warn`(agmsg が導入済みならバージョン違いでも warn)、`AGMSG_STORAGE_PATH` があればその値で判定、agmsg 未導入 / 読めない config / 型違い / codex home 解決不能 → `info`。`countFailed` の対象にならない(既存の exit code テストが pass のまま)
