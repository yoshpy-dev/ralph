...
- Pipeline cycle 2/2: cross-review cycle-1 の ACTION_REQUIRED 2 件(AR#1 doctor --strict がプランニングエラーで fail-open、AR#2 settings スナップショット削除が (e) manifest/実体整合をすり抜け)を `e01939e` で修正。同コミットで self-review cycle-1 も再走し、cycle-2 の新規所見 5 件(C2-M1/M2、C2-L1〜L3)を `e39d436` で修正。残り 2 件(C2-L4 drift 案内文言の 4 箇所重複、C2-L5 cycle-1 レポートの行番号ポインタが e01939e の diff で下方シフト)は当時 cap 2/2 到達につき batched tech-debt 行として記録(`docs/tech-debt/README.md`)。
- Pipeline cycle 3(操作者承認で cap 2→3): cross-review cycle-2 の ACTION_REQUIRED 4 件(settings/block 検査エラーの fail-open ×2、status の corrupt-manifest 隠蔽、purity ガードのパス走査欠落)を `25b4f79` で修正(warn 返却経路の全数監査を含む)。監査の残余(conflict scan の無音 skip — 実際は planner 経路が先に捕捉する shadowed ケース)は防御的修正+ゲートレベル回帰テストとして `fd136ae`。self-review cycle-3 の新規所見(C3-M1 + C3-L1〜L7)は本 cycle 内で修正(C3-L7(a)(b) のレポート内ポインタ decay は歴史的成果物のため能動修正せず、cycle-3 レポート群が上書き/追記で解消)。C2-L5 の deferral 判断は cap 引き上げ後も維持(理由の記録を tech-debt 行で更新済み)。C2-L4 は cycle 4 の drift 案内変更(de36e45)がトリガーを満たしたため `writeDriftGuidanceV2` への集約で解消。
- Pipeline cycle 4(操作者承認で cap 3→4): cross-review cycle-3 の ACTION_REQUIRED 2 件(AR#1 未追跡パスの drift 案内が eject/adopt 対象外である旨を伝えていない、AR#2 codex config コメントに session/phase 履歴が残存)を `de36e45` で修正(`writeDriftGuidanceV2` ヘルパへの集約と `(untracked)` 注記の追加を含む。上記 C2-L4 の解消もこのコミット)。self-review cycle-4 の新規所見(C4-M1/M2、C4-L1〜L7)を `04472d9` で修正。tester cycle-4 は 1 件の既知ギャップを記録: `04472d9`(C4-M1)が追加した `internal/cli/doctor.go` の `checkScaffoldCoreHashes` 未追跡 drift 分岐に専用テストがない(メッセージ整形のみで制御フローリスクなし、低severity、cap 4/4 到達につき tech-debt 行候補として記録に留める)。
