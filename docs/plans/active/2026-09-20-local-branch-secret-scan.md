...
- [ ] AC-3b: base と同じ名前の tag や、`origin/<base>` という名前のローカル branch があっても、merge-base は存在を確認した branch の ref から取る(cross-review cycle 1 の AR-2)
- [ ] AC-2d: HEAD の `.gitallowed` が symlink のとき、コミット済みの tree の中のリンク先を allowlist として読む。リンク先が読めない場合は空の allowlist で scan し通知を出す(cross-review cycle 1 の AR-1)
