...
- [x] AC-4: `templates/base/ralph.toml` の `[org.permissions]` コメントに `docs/tech-debt/README.md` への参照がなく(`grep -c 'tech-debt' templates/base/ralph.toml` が 0)、recipe へのポインタがある。`internal/org/permissions.go` に検証日・codex バージョン・evidence への参照があり、`git diff main -- internal/org/permissions.go` の非コメント差分は fail-closed エラー文言 2 行(「requires [org.permissions].codex_verified=true … docs/recipes/codex-seat-permissions.md」)のみでロジック不変。`internal/config/config.go` の `OrgPermissionsConfig` / `CodexVerified` の doc も同じ位置づけで、`tech-debt` への参照なし。`/org` skill の permission 作法に「暫定制約」の語がなく(`grep -c '暫定制約' .claude/skills/org/SKILL.md` が 0)、`codex_verified` と recipe への言及がある。4 面 `cmp` 一致、`check-skill-sync.sh` pass
