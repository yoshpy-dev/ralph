...
- [x] AC-5: デッドマン: ALERT 後 `deadman_minutes` 無応答(3 情報源 OR)または Lead 自身の異常で `escalations.jsonl` 追記+stderr(+darwin 通知 best-effort)(短縮値でテスト)。Evidence: `checkDeadman`, `realEscalate`(`osascript`, `%q` 引用); test: `TestWatch_Deadman_NoActivity_EscalatesOnceAfterTimeout`, `TestWatch_Deadman_LeadActivity_PreventsEscalation`, `TestWatch_Deadman_LeadIsAnomalySubject_EscalatesImmediately`。**manifest 情報源(最終モデル、cycle 3 で確定)**: `leadActivityEventCount` は `org_id` スコープ(AR-1)+ 全 `sent` イベント無条件カウント(`sent` は `ralph org send` 経由のみ生成されるため lead-authored by construction — H3-1 で `SeatID==LeadIdentity` の逆転判定を修正)+ lead 主導のライフサイクルイベント集合(`spawned`/`spawn_started`/`stopped`/`disbanded`/`rejected`、watchdog 自身の遮断書き込みを除く — M3-1 で `spawned`/`spawn_started`/`rejected` を追加)を数える。agmsg history 情報源も lead 発言のみを対象にフィルタ(`filterLeadHistoryLines`)し、件数比較(`HistoryLeadLines`、旧 `History` 文字列比較から変更 — ウィンドウ退避による偽陽性を解消)。tech-debt 行(下記、行 70)は fully closed。既知の残課題なし。test: 上記に加え `TestFilterLeadHistoryLines`, `TestWatch_Deadman_LeadHistoryLine_ClearsPendingAlert`, `TestWatch_Deadman_WatchdogAlertHistoryLine_DoesNotClearPendingAlert`, `TestWatch_Deadman_SeatSentEvent_ClearsPendingAlert_WatchdogCutoffDoesNot`, `TestWatch_Deadman_LeadSpawnsReplacementSeat_ClearsPendingAlert`, `TestWatch_Deadman_HistoryWindowEviction_DoesNotFalselyClearPendingAlert`。
...
- **Cycle 3(cross-review cycle-2 #3/#4 修正 + self-review cycle-3 追加修正、cap を 3 に引き上げ)**: cross-review cycle-2 の #3(P1: デッドマンの agmsg history 情報源が lead 以外の全チーム発言で pending alert を消してしまう)/#4(P2: total budget 遮断が同一サイクル内で直前に遮断した座席を古いスナップショットで再評価し、スプリアスな ALERT/デッドマン登録を生む)を `764b01f` で修正(`filterLeadHistoryLines`/`leadHistoryFromField` で history を lead 発言のみにフィルタ、`evaluateTotalBudget` が返す `cutSeats` で同一サイクル内の再評価をスキップ)。続く `d6e557a` は `strings.Cut`/`SplitSeq` への書き換えで振る舞い変更なし(lint cleanup)。self-review cycle-3(`ae6d852` 記録、findings H3-1/M3-1/M3-2/L3-1..3)が `764b01f` 由来の `leadActivityEventCount` 修正を精査し、`ev.SeatID == LeadIdentity` 判定が逆転していたことを発見(H3-1: `Send` は `SeatID: p.To` = 受信者を書くため、seat→lead の `sent` は除外され lead→seat の `sent` がカウントされる誤りだった)。`8bbd55e` で (a) `sent` イベントを無条件カウントするモデルへ修正(`ralph org send` は lead/operator のみが叩く検証済み前提により sent = lead-authored by construction)、(b) lead 主導ライフカウント対象を `stopped`/`disbanded` から `spawned`/`spawn_started`/`stopped`/`disbanded`/`rejected` へ拡張(M3-1: ALERT に応答した lead の代替座席 spawn を活動として認識)、(c) history 情報源を文字列比較から件数比較(`HistoryLeadLines`)へ変更(M3-2: ウィンドウ退避による偽陽性の解消)。`watchPendingAlert.History string` → `HistoryLeadLines int` は `watch-status-<org_id>.json` の破壊的フィールド改名だが、watch はまだ本番投入前でランタイムローカルな状態のため移行パス不要(次回起動でクリーンに再構築)。tech-debt 行 70 を最終モデルに合わせて書き換え、行 71(deferred LOW)に L2-6(zero-active-seats guard が `Active` を未クリアのまま残す)を 4 件目として追加。詳細: `docs/reports/verify-2026-08-02-org-runtime-watchdog.md` cycle-3 節、`docs/reports/test-2026-08-02-org-runtime-watchdog.md` cycle-3 節。
