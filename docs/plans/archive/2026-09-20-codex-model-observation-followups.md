...
- [x] AC-7: 既存のテスト(#165 の観測・配線・CLI・doctor)が変更なしの意図のまま通る。`./scripts/run-verify.sh` と `./scripts/run-test.sh` が green。PR 本文に `Closes #173`
...
- 2026-09-20 sync-docs cycle 2(最終 cycle): cycle 2 の verify/test はいずれも、sync-docs cycle 1 で追記した節(stop の 1 回の観測が spawn と同じ最大 8 秒の上限を持つ)が HEAD で引き続き正確で 8 面すべて一致していることを確認済みのため、8 面とも再編集なし。tech-debt は 2 件検討: (a) `observeStopModelReceipt` の `o.Receipts.Read()` 失敗枝が未テスト(test cycle 2 の Test gaps)— この関数の他の読み取り失敗枝と同型の、この PR 固有ではない一般的な未検証 I/O 枝であり(cycle 1/2 の test report がすでに同じ性質の枝を繰り返しそう特徴づけている)、既存の codex-observer 行の主題(codex 自身の未文書化フォーマットが将来変わるリスク)とは別物なので、その行への追記はせず、test report の Test gaps 節をそのまま記録として残す判断とした。(b) `TestOrgSpawn_Codex_OldSessionRecordNotPickedOnRespawn` の 50ms 上限問題(self-review C2-L2)は 711cd9d で該当テストのコメントに決定的な pin(`TestObserveCodexEffectiveModel_OldSessionUpdatedLaterNotPicked`)を明記済みで、cycle 2 の test report が再確認して「新しい指摘ではない」としている — 未解決ではないため記録不要。test timing/fixture の作法を説く既存ドキュメント(`.claude/rules/ralph/testing.md`、`.claude/rules/ralph/golang.md`、`docs/quality/*.md`)はいずれも test timing/fixture の話題を扱っていないため(grep 確認済み)新規追記はせず、`testCodexObserveGenerousBudget` 脇のコード comment を文書とする判断を維持。`#173`/`AR-3`/`AR-4` を「未解決」の含みで参照する箇所は docs/reports・docs/plans・internal/org/ 内の由来コメント以外になし(全リポジトリ grep)。詳細は sync-docs report の Cycle 2 節
- 2026-09-20 cycle 2 の結果: verify pass(4c0feb6)、test pass(97a4429。race 100 回を 3 通りで失敗 0、走査への 3 / 10 / 25 / 60ms の遅延注入でも失敗 0、30 秒の定数と stop 前の上限の引き上げが効いていることを red/green で確認)、sync-docs(5bf9886。文書の変更なし)、cross-review cycle 2 は指摘 0 件(b83f2ba)。push 前に `./scripts/run-verify.sh`、`TMPDIR=/tmp` の Go テスト、履歴込みの range secret scan を実行してすべて pass。PR #175 を作成(`Closes #173`)
