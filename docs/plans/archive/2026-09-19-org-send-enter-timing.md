...
| # | 変更 | ファイル | 内容 |
|---|------|---------|------|
| 1 | 待ち | `internal/org/verbs.go`(`Send`) | `PaneSendText` の後、`PaneSendKeys("Enter")` の前に待つ。既定 750ms(定数 `defaultSendEnterDelay`)。`SendParams.EnterDelayMS` が正ならその値。テスト用に `Org.SendEnterDelay`(ゼロなら既定。`AgentStartRetryInterval` と同じ形)で上書きできる。待ちは ctx の期限を尊重する。**self-review による改訂**: 座席の idle / done の待ちが終わった時点で `--timeout-ms` の残りが待ち時間以下なら、本文を入力する前にエラーで終わる(`nothing was typed`)。入力した後、Enter を送る前に失敗した場合(待ちの途中の期限切れ、Enter の送信エラー)は `SendResult` に `TextTyped` と `PaneID` を載せ(`EnterPressed` は false)、CLI が「その pane を確認してから送り直す」よう stderr に案内する(送り直すと入力欄の残骸の後ろに 2 通目が入力されるため)。Enter を送った後の失敗(`sent` イベントの記録エラー)は `EnterPressed` が true で、CLI は「submit は済んでいるが記録できなかった。送り直さない」と別の案内を出す。submit 済みの座席に「未送信なので submit せよ」と案内すると、承認ダイアログに進んだ座席で人が盲目的な Enter を押すことになるため(再確認 NEW-1)。**cross-review cycle 1 による改訂(AR-1)**: herdr の呼び出しは ctx の期限で打ち切られる(`exec.CommandContext`)ので、呼び出しがエラーで返っても、その操作が pane に届いていないとは言えない。`TextTyped` / `EnterPressed` の 2 つの bool をやめ、`SendResult.Progress`(`SendProgress` 型)の 5 状態で「どこまで進んだか」を返す: 何も送っていない / send-text を呼んだが結果不明 / 本文入力済みで Enter は未試行 / Enter を呼んだが結果不明 / Enter 送信済み。結果不明の 2 状態では「未送信」と断定せず、CLI は「届いたかどうか分からない。先に pane を読み、入力欄に本文が残っているときだけ submit か消去をする。それ以外(working、ダイアログ)なら Enter を押さず、送り直さない」と案内する |
...
| 4 | CLI | `internal/cli/org.go`(`send`) | `--enter-delay-ms`(既定 0 = 組み込みの既定値 750ms。負の値は拒否)。submit を確認できなかった場合は stderr に注意を出す(exit code は 0 のまま): 座席の pane を `ralph org read` で確認し、本文が入力欄に残っている場合に限って `herdr pane send-keys <pane_id> Enter` を送る、という案内と pane_id。**cross-review cycle 1 による改訂(AR-2)**: 警告と失敗時の案内に出す `ralph org read` のコマンドは、`--state-dir` が明示されていたときだけ解決済みの絶対パスで `--state-dir` を付ける(付けないと既定の manifest を引き、seat not found になるか別の座席を読む)。環境変数での指定は同じシェルなら引き継がれるので付けない |
...
- 2026-09-19 sync-docs(cycle 1、`doc-maintainer`): `/org` skill の `send` 行(4 面)と recipe(2 コピー)に、Slice D/E で足された挙動 2 点を追記(`--timeout-ms` の残りが Enter 前の待ちを賄えないときは何も入力せずエラーで終わること、エラー終了時は「typed のみ」と「Enter 済み」で案内が分かれ、後者は送り直さないこと)。同期ゲート 3 本 pass。`docs/tech-debt/README.md` に 1 行追加: test report(`docs/reports/test-2026-09-19-org-send-enter-timing.md`)が名指しした 2 つのカバレッジ欠落(`findSeat` の read-error 経路、750ms 既定値がランタイムで実際に使われることを検証するテストがないこと)を記録。`docs/specs/2026-08-01-org-runtime.md`、`README.md`、`AGENTS.md`、`.claude/rules/ralph/agent-messaging.md`、`internal/org/prompts/`、既存の tech-debt 行は現状の挙動と矛盾する記述が無いことを確認し、変更なし
- 2026-09-19 cross-review cycle 1(`docs/reports/cross-review-triage-org-send-enter-timing.md`、2309da9): Codex の指摘 2 件(どちらも P2)を ACTION_REQUIRED と判定。AR-1 Enter の呼び出しが期限で打ち切られると、届いていても「未送信」と案内する(再確認 NEW-1 と同じ種類の問題。同じ理由で send-text の失敗も「入力されていない」とは言えないため、あわせて 5 状態に整理する)。AR-2 案内の `ralph org read` に `--state-dir` が入らない。ユーザー決定(AskUserQuestion): 2 件とも修正して cycle 2/2 としてパイプラインを再実行
- 2026-09-19 work(cycle 2): Slice F は implementer に委譲(35a3594、4 ファイル)。`SendResult.TextTyped` / `EnterPressed` を `Progress`(`SendProgress` の 5 状態)に置き換え、CLI は状態ごとに 4 種類の案内を出す(何も送っていない場合は案内なし)。send-text の失敗は挙動も変わった: これまでは案内なし・`PaneID` 空だったが、「本文が届いたか分からない」という案内と `PaneID` を返す。`ralph org read` の案内は 1 つの helper で組み立て、`--state-dir` が明示されたときだけ解決済みの絶対パスを付ける(空白などを含むパスは単一引用符で囲む)。`--config` は `read` の座席の検索に影響しないことをコードで確認。CLI の stub に send-text の応答を遅らせる環境変数を足し、「本文入力済み・Enter 未試行」の案内も CLI 経由でテストした(1 回あたり約 1.8 秒)。orchestrator が HEAD 一致・porcelain 空・差分を確認し、skill 4 面と recipe 2 コピーの文言を新しい案内に合わせた(f9f0d82)。同期ゲート 3 本と send のテストは pass
- 2026-09-19 self-review cycle 2(同 report の Cycle 2 節、6a7e2ae): MERGE、MEDIUM 1 / LOW 5。5 状態と 4 つの案内はいずれも Send が観測した範囲だけを述べ、どの経路にも「無条件に Enter を押せ」という案内はない、と確認された。全件を同 cycle 内で修正(最終 cycle のため再確認は行わず、orchestrator が差分を読んだ)。C2-1 CLI 経由で「本文入力済み・Enter 未試行」を起こすテストの余裕が stub の起動コストと同じ幅しかなく、速いマシンほど薄い → `--timeout-ms 2500 --enter-delay-ms 1800`(sleep は 1.1 秒のまま)にして、ctx が 400ms 以上の差で勝つようにした。15 / 15 pass、1 回あたり約 2.8 秒(reviewer の「実行時間は変わらない」は誤りで、implementer の実測では約 1 秒増)。C2-2 exit 0 の未確認警告に「それ以外なら Enter を押さない」を明示。C2-3 テスト名に残った `Submitted` を改名。C2-4 `SendProgress` の doc から、存在しない 2 つの bool の経緯を外し、結果が不明になる理由(`exec.CommandContext`)を先頭に。CLI の各 case のコメントは案内に固有の 1〜2 行に縮めた。C2-5 skill 4 面と recipe 2 コピー: pane に何も送る前の失敗では note が出ない(そのまま送り直せる)ことを追記(8a78aca、orchestrator)。C2-6 `--state-dir` を付けない根拠は cwd と環境が変わらない前提であることを doc comment に明記。コードは implementer(646fb4b、4 ファイル)
- 2026-09-19 cycle 2 の結果: verify pass(2ef6a5d。tech-debt の行に残っていた改名前のテスト名を 9daabc0 で修正)、test pass(fda0c20。red/green 5 種類、負荷下の実プロセスのテスト 10 / 10、`SendProgress.String()` のテストを追加)、sync-docs(968f21a。skill と recipe の締めの文が「入力欄に残っているときだけ」を全状態に当てはめていたのを、note に従う形に修正。tech-debt の行に残り時間の切り上げの枝を追記)、cross-review cycle 2 は指摘 0 件(5d9df97)。push 前に `./scripts/run-verify.sh`、`TMPDIR=/tmp` の Go テスト、履歴込みの range secret scan を実行してすべて pass。PR #172 を作成
