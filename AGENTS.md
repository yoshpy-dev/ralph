...
- `.ralph/local/` — downstream extension points (`hooks/<event>.d/`, `verify.d/`, `test.d/`) that `scripts/run-verify.sh` and the hooks dispatcher execute after core processing (`hooks/<event>.d/` wiring is Claude Code today; Codex's `.codex/config.toml` still calls hook scripts directly — Phase 5 tech debt)
