...
# description, policy) each fail closed, that a clean fixture passes, and that
# the intentional Claude/Codex PR provenance example variance is normalized.
#
...
# ── G. driver-specific PR provenance examples are normalized ────────────────
G_DIR="$(mktemp -d)"
...
run_case "G. driver-specific PR provenance examples" 0 "$G_DIR"
rm -rf "$G_DIR"
